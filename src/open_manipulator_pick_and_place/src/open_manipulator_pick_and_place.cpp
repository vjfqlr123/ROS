...
void OpenManipulatorPickandPlace::publishCallback(const ros::TimerEvent&)
{
  ...
  if(!(node_handle_.getParam("/wall_arrive_end", wall_arrive_end)))           // here wall_arrive_end = 1 -> start pick
  {
...
{
  std::vector<int> wall_arrive_end;
...
void OpenManipulatorPickandPlace::demoSequence()
{
  ...
  case 3: // pick the box
    ...
    for(int i = 0; i < ar_marker_pose.size(); i ++)
    {
      if(ar_marker_pose.at(i).id == 0)
      {
        ...
        kinematics_position.push_back(0.03);
  ...
  case 5: // initial pose
    ...
    setJointSpacePath(joint_name_, joint_angle, 1.0);
    demo_count_ ++;
    node_handle_.setParam("/pick_end", 1);          // here     pick_end = 1 -> turtlebot move back
    break;
  case 6: // place pose
