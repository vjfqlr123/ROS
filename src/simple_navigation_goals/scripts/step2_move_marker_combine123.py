...
class Move2Marker:
    ...
    def __init__(self):
        ...
        self.step7_get_marker_end = False
        
    def get_marker_info(self, msg):
        
        if self.step7_get_marker_end == False:
            ...
            if len(msg.markers) > 0:
                ...
                if self.step6_lost_marker == True and self.step5_move_2_marker_end == False:
                    self.tw.linear.x = 0
                    self.tw.angular.z = 0
                    self.pub.publish(self.tw)
    ...
    def get_scan_info(self, msg):
        ...
        if self.step5_move_2_marker_end == True:
            if self.dist.data < LIDAR_MAX_DIST:
                print("==================")
                self.tw.linear.x = 0
                self.tw.angular.z = 0
                self.pub.publish(self.tw)
            ...
            elif self.dist.data > LIDAR_MAX_DIST:
                print(self.dist.data)
                self.tw.linear.x = ANG_SPEED / 3
                self.pub.publish(self.tw)        
...
if __name__ == '__main__':
    try:
        start = rospy.get_param("/slam_navigation/navigation_end")
